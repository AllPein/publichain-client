...
export const getBaseApiUrlWithoutProtocol = (): string => {
  ...
  return import.meta.env.VITE_BASE_API_URL.replace('http://', '').replace(
    'https://',
    '',
  );
};
...
export async function createWebsocket({
  ...
  onmessage,
  dispatch,
  backendPath = import.meta.env.VITE_BASE_API_URL,
  countReconnect,
  ...
  dispatch(WebsocketAction.setWebsocket({ ws, backendPath }));
